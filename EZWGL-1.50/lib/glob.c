     ...
     char *dir;
{
  ...
  while (!skip)
	...
	continue;
      ...
      {
	char *tptr = dp->d_name;
	if(tptr[0] == '.' &&  ( tptr[1] == '\0' || (tptr[1]== '.' && tptr[2]== '\0'))) continue;
      }
...
char **EZ_GlobFileName(fname)
     char *fname;
{
  if(fname)
    {
      char **ans = glob_filename(fname);  
      if(ans != NULL && ans != &glob_error_return)
	return(ans);
    }
  return(NULL);
}

char **EZ_GlobFiles(pattern, showDotFiles) char *pattern; int showDotFiles;
{
  if(pattern)
    {
      char **ans;
      int  save = noglob_dot_filenames;

      noglob_dot_filenames = (showDotFiles == 0);
      ans = glob_filename(pattern);  
      noglob_dot_filenames = save;
      if(ans != NULL && ans != &glob_error_return)
	return(ans);
    }
  return(NULL);
...
int EZ_GlobHiddenFiles(bl) int bl;
{ 
  int s = (noglob_dot_filenames == 0);
  noglob_dot_filenames = (bl == 0); 
  return(s);
}
